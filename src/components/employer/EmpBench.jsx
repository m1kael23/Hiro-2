...
import { notifyBenchView, notifyBenchInterest } from '../../services/notificationService';

// Static Bench candidate cards — real data will come from Firestore
// once the Bench browse query (users where bench_active=true + DNA match) is built.
// Each card carries a `candidateId` placeholder for notification wiring.
const BENCH_CANDIDATES = [
  {
    id: 'bench_sc',           // replace with real Firestore uid when live
    ...
    reloc: true,
    insight: {
      style: { background: 'rgba(13,148,136,.08)', border: '1px solid rgba(13,148,136,.2)', color: 'var(--teal)' },
      text: '🪑 Previously matched · Expressed interest · Process stalled — now open from Aug 2025',
    },
    btnLabel: 'Express interest →',
    btnStyle: { background: 'rgba(13,148,136,.2)', border: '1px solid rgba(13,148,136,.35)', color: 'var(--teal)' },
    jobTitle: 'Sr PM',
  ...
  },
  {
    ...
    reloc: false,
    btnLabel: 'Save interest →',
    btnStyle: { background: 'rgba(108,71,255,.2)', border: '1px solid rgba(108,71,255,.35)', color: '#a78bfa' },
    jobTitle: 'Lead PM',
...
export default function EmpBench() {
  ...
  // Track which cards have been viewed / had interest expressed
  const [viewed,    setViewed]    = useState({});
  const [interested, setInterested] = useState({});

  // Fire bench_view notification when a card is expanded / focused
  async function handleView(cand) {
    if (viewed[cand.id]) return; // only fire once per session
    setViewed(p => ({ ...p, [cand.id]: true }));
    await notifyBenchView({ candidateId: cand.id, companyName });
  }

  // Fire bench_interest notification + show toast when employer expresses interest
  async function handleInterest(cand) {
    if (interested[cand.id]) return;
    setInterested(p => ({ ...p, [cand.id]: true }));
    try {
      await notifyBenchInterest({
        candidateId: cand.id,
        companyName,
        jobTitle: cand.jobTitle,
      });
      showToast(`Interest expressed — ${cand.initials}. will be notified when ready 🎉`, 'success');
    } catch (err) {
      console.error('[EmpBench] notifyBenchInterest failed:', err);
      ...
      <div className="scroll">
        <div className="review-shell" style={{ maxWidth: 820 }}>
          ...
          <div className="g4" style={{ marginBottom: 18 }}>
            ...
            <div className="stat-tile" style={{ '--glow': 'rgba(34,197,94,.25)' }}><div className="stat-eyebrow">Tier 1 (prev match)</div><div className="stat-val" style={{ color: 'var(--green)' }}>3</div><div className="stat-label">Already know {companyName}</div></div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {BENCH_CANDIDATES.map(c => (
              <div
                key={c.id}
                className="bench-cand-card"
                style={{ borderColor: c.borderColor, background: c.bg2 }}
                onMouseEnter={() => handleView(c)}
              >
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                  <div style={{ width: 40, height: 40, borderRadius: 12, background: c.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Manrope',sans-serif", fontSize: 14, fontWeight: 800, color: '#fff', flexShrink: 0 }}>
                    {c.initials}
                  </div>
                  <div style={{ flex: 1 }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6, flexShrink: 0 }}>
                    <button
                      ...
                      style={{
                        ...c.btnStyle,
                        opacity: interested[c.id] ? 0.5 : 1,
                        cursor:  interested[c.id] ? 'default' : 'pointer',
                      }}
                      onClick={() => handleInterest(c)}
                      disabled={interested[c.id]}
                    >
                      {interested[c.id] ? '✓ Interest sent' : c.btnLabel}
                    </button>
