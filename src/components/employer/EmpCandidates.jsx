...
// ─── DNA bar row (candidate vs team target) ─────────────
...
function CandDetail({ cand, job, onStageChange, onMessage, onMatchBack }) {
  ...
  const [tab,   setTab]   = useState('overview'); // 'overview' | 'dna' | 'experience'
        ...
        <div style={{ position:'absolute', inset:0, background:'rgba(10,10,15,.9)', zIndex:100, display:'flex', alignItems:'center', justifyContent:'center', padding:20, backdropFilter:'blur(4px)' }}>
          <div style={{ background:'var(--card)', border:'1px solid var(--border)', borderRadius:'var(--r)', padding:24, maxWidth:320, width:'100%', boxShadow:'0 20px 40px rgba(0,0,0,.4)' }}>
            ...
            <p style={{ fontSize:13, color:'var(--text2)', lineHeight:1.5, marginBottom:20 }}>
              Are you sure you want to move <strong>{cand.name}</strong> from <strong>{stage}</strong> to <strong>{confirmStage}</strong>?
            </p>
            <div style={{ display:'flex', gap:10 }}>
              <button 
                onClick={() => setConfirmStage(null)}
                style={{ flex:1, padding:'10px', borderRadius:'var(--rp)', background:'rgba(255,255,255,.05)', border:'1px solid var(--border)', color:'#fff', fontSize:13, fontWeight:600, cursor:'pointer' }}
              >
                Cancel
              </button>
              <button 
                onClick={confirmStageChange}
                style={{ flex:1, padding:'10px', borderRadius:'var(--rp)', background:'var(--violet)', border:'none', color:'#fff', fontSize:13, fontWeight:600, cursor:'pointer' }}
              >
                Confirm
              </button>
            </div>
...
export default function EmpCandidates() {
  ...
  // Fetch employer's jobs + all match applications + candidate profiles
  useEffect(() => {
    if (!profile?.id) return;

    // 1. Live jobs for this employer
    ...
    const unsubJobs = onSnapshot(qJobs, (snapshot) => {
      ...
      console.error('EmpCandidates: jobs snapshot error', err);
    });

    // 2. All match/application documents for this employer
    const qApps = query(collection(db, 'applications'), where('employerId', '==', profile.id));
    const unsubApps = onSnapshot(qApps, (snapshot) => {
      const apps = snapshot.docs.map(d => ({ id: d.id, ...d.data() }));
      setApplications(apps);
    }, (err) => {
      console.error('EmpCandidates: applications snapshot error', err);
      setLoading(false);
    });
  ...
  const scoredCandidates = useMemo(() => {
    ...
    return candidates
      ...
      .filter(Boolean)
      .sort((a, b) => {
        // Candidates who expressed interest bubble up, then by match score
      ...
      <div className="view" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ textAlign: 'center', maxWidth: 360 }}>
          ...
          <div style={{ fontSize: 13, color: 'var(--text3)', marginBottom: 20 }}>
            Post a role to start seeing DNA-matched candidates.
          </div>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden', flexDirection: 'column' }}>
        ...
        <div style={{ padding: '10px 16px', borderBottom: '1px solid var(--border)', flexShrink: 0, display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <select
            className="sel"
            value={selectedJobId || ''}
            onChange={e => { setSelectedJobId(e.target.value); setSelectedId(null); setFilter('All'); }}
            style={{ flex: 1, maxWidth: 340, fontWeight: 600 }}
          >
            {dbJobs.map(j => (
              <option key={j.id} value={j.id}>{j.title} · {j.companyName || profile?.company_name || ''}</option>
            ))}
          </select>
        ...
        <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '8px 10px' }}>
              {visible.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '40px 0', color: 'var(--text3)', fontSize: 12 }}>
                  {filter === 'All'
                    ? 'No matched candidates yet. The engine runs when candidates set their DNA.'
                    : `No candidates in "${filter}" filter.`}
                </div>
              ) : visible.map(c => (
                <CandRow
                  key={c.id}
          ...
          {selected ? (
            <CandDetail
              ...
              onMatchBack={handleExpressInterest}
            />
          ) : (
